...
public class Database {
    ...
    /**
     * <p>Connects to the server or to the database specified by the parameter
     * <code>url</code>.</p> <p>The
     * <code>url</code> should be of the form
     * <code>jdbc:subprotocol:subname</code>, where
     * <code>subprotocol</code> is the protocol used by the Driver used to
     * connect to the server or database
     * <code>subname</code>. If it is not know which
     * <code>subprotocol</code> should be used, it's better to use the method
     * <code>getUnknownConnection(String url, String user, String password)</code>,
     * although it is called by default after the failed first try of
     * establishing the connection.</p> <p>Note that not all server or database
    ...
    /**
     * <p>Connects to the server or to the database specified by the parameter
     * <code>url</code>.</p> <p>This method tries to establish a connection to
     * the
     * <code>url</code> server or database by trying to find itself the
     * subprotocol used by the server.</p> <p>Note that not all server or
     * database are supported by this class. For the supported drivers' list,
     * use
     * <code>getSupportedDrivers()</code> method.</p>
     *
     * @param url a server or database url
     * @param user the username of the server or database
     * @param password - the password used to log in the server or database
     * using the account of <code>user</code>
     * @return the Connection Object to the database or server specified by      * the <code>url</code>
     ...
     * @see manuzioParser.Database#getSupportedDrivers()
     */
    private static Connection getUnknownConnection(String url, String user, String password) throws SQLException {
    ...
    /**
     * <p>Gets the currently supported database drivers .</p>
     *
    ...
    public static Connection buildManuzioDB(String url, String dbName, String user, String password, boolean override) throws SQLException {
        ...
        Connection conn = getConnection(url, user, password); //connects to the server
        ...
        try {	//builds the db
            conn = getConnection(url + "/" + dbName, user, password); //connects to db just created
            query = conn.createStatement();
